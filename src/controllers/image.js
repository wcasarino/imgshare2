const path = require('path');
const { randomNumber } = require('../helpers/libs');
const fs = require('fs-extra');
const md5 = require('md5');

const { Image, Comment, User } = require('../models');
const sidebar = require('../helpers/sidebar');

//const { user } = require('../server/config')

const ctrl = {};

ctrl.index = async (req, res) => {
    let viewModel = { image: {}, comments: {}, userName: String, mismoUser: Boolean};
    
    const image = await Image.findOne({filename: {$regex: req.params.image_id}});
    const user = req.user;
    if (image) {
        image.views = image.views + 1;
        viewModel.image = image;
        await image.save();
        const comments = await Comment.find({image_id: image._id}).sort({timestamp: -1});
        viewModel.comments = comments;
        const userImg = await User.findOne({_id: image.user});
        viewModel.userName = userImg.name;
        viewModel.mismoUser = false;
        if (user) {
            if (user.id == userImg.id) {
                viewModel.mismoUser = true;
            }
        }
        viewModel = await sidebar(viewModel);
        console.log(viewModel);
        console.log(user);
        //console.log(req.params.user.name);

        res.render('image', viewModel);
    } else {
        res.redirect('/');
    }
};

ctrl.createG = (req, res) => {
    res.render('upload'); 
};

ctrl.createP = (req, res) => {

    const saveImage = async () => {
        const imgUrl = randomNumber();
        //console.log(imgUrl);
        const images = await Image.find({filename: imgUrl});
        if (images.length > 0) {
            saveImage();
        } else {
            //console.log(imgUrl);
            const imageTempPath = req.file.path;
            const ext = path.extname(req.file.originalname).toLowerCase();
            const targetPath = path.resolve(`src/public/upload/${imgUrl}${ext}`);
            
            if (ext === '.png' || ext === '.jpg' || ext === '.jpeg' || ext === ".gif") {
                await fs.rename(imageTempPath, targetPath);
                const newImg =  new Image({
                    title: req.body.title,
                    filename: imgUrl + ext,
                    description: req.body.description,
                    user: req.user.id                    
                });
                const imageSaved = await newImg.save();
                res.redirect('/images/' + imgUrl);
            } else {
                await fs.unlink(imageTempPath);
                res.status(500).json({error: 'Only Images are allowed'});
            }
        }
    };
    saveImage();
};

ctrl.like = async (req, res) => {
   const image = await Image.findOne({filename: {$regex: req.params.image_id}}) 
   //console.log(image.likes);
   if (image) {
       image.likes = image.likes + 1;
       await image.save();
       res.json({likes: image.likes});
   } else {
       res.status(500).json({error: 'Internal Error'});
   }
};

ctrl.comment = async (req, res) => {
    const image = await Image.findOne({filename: {$regex: req.params.image_id}});
    if (image) {
        const newComment = new Comment(req.body);
        //newComment.gravatar = md5(newComment.email);
        newComment.gravatar = md5(req.user.email);
        newComment.name = req.user.name;
        newComment.user = req.user.id;
        newComment.image_id = image._id;
        await newComment.save();
        res.redirect('/images/' + image.uniqueId);
    } else {
        res.redirect('/');
    }
};

ctrl.remove = async (req, res) => {
    const image = await Image.findOne({filename: {$regex: req.params.image_id}});
    if (image) {
        await fs.unlink(path.resolve('./src/public/upload/' + image.filename));
        await Comment.deleteMany({image_id: image._id});
        await image.remove();
        res.json(true);
    }
};

module.exports = ctrl;